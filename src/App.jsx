      ...
      <div className="relative w-full max-w-sm bg-white/95 backdrop-blur-2xl rounded-[2.5rem] p-8 shadow-2xl border border-white animate-in zoom-in-95 duration-200">
        <div className="flex flex-col items-center text-center">
          ...
          <div className="flex gap-3 w-full">
            <button onClick={onClose} className="flex-1 py-4 bg-slate-100 text-slate-500 rounded-2xl font-black text-sm hover:bg-slate-200 transition-colors">{cancelText}</button>
            <button onClick={() => { onConfirm(); onClose(); }} className={`flex-1 py-4 rounded-2xl font-black text-sm text-white shadow-lg transition-all active:scale-95 ${type === 'danger' ? 'bg-rose-500 shadow-rose-200 hover:bg-rose-600' : 'bg-blue-600 shadow-blue-200 hover:bg-blue-700'}`}>{confirmText}</button>
          </div>
        </div>
      </div>
    </div>
    ...
    <div className={`${THEME.card} rounded-[2rem] overflow-hidden shadow-2xl shadow-blue-900/5`}>
      <div className="p-5 flex justify-between items-center border-b border-slate-50 bg-slate-50/30">
        ...
        <div className="flex items-center gap-1">
          <button onClick={handleRematchParticipation} className="flex items-center gap-1 px-3 py-2 hover:bg-white rounded-xl transition-colors group text-[12px] font-bold" title="명단 보정">
            <ScanSearch size={14} className="text-slate-300 group-hover:text-blue-500" />
            <span className="text-slate-400 group-hover:text-blue-500">보정</span>
          </button>
...
const AdminView = ({ 
  ...
  const members = useMemo(() => Object.entries(masterData.members).sort((a, b) => {
  ...
  useEffect(() => {
    if (lastActionTarget && itemRefs.current[lastActionTarget]) {
      itemRefs.current[lastActionTarget].scrollIntoView({ behavior: 'smooth', block: 'center' });
      const input = itemRefs.current[lastActionTarget].querySelector('input');
      if (input) input.focus();
    }
  }, [lastActionTarget, members]);
...
const App = () => {
  ...
  const handleUpdateMasterEntry = (oldName, field, value) => {
    ...
    if (field === 'name') {
      ...
      setLastActionTarget(value);
    } else {
      newMembers[oldName] = { ...newMembers[oldName], [field]: value };
      setLastActionTarget(oldName);
    }
    setMasterData({ ...masterData, members: newMembers });
  };

  // --- Participation Logic ---
  const handleUpdateParticipationName = (oldName, newName) => {
    const newList = scannedData[activeTime].map(p => {
      if (p.name === oldName) {
        const masterInfo = masterData.members[newName.trim()];
        return masterInfo 
          ? { name: newName, rank: masterInfo.rank, type: masterInfo.type }
          : { ...p, name: newName, type: "미등록" };
      }
      return p;
    });
    ...
    if (modal.isOpen && modal.title === '신규 유저 추가') {
      setModal(prev => ({
        ...
        onConfirm: () => {
          ...
          const newMembers = { [nickname.trim()]: { rank: "R3", type: "본캐" }, ...masterData.members };
          setMasterData({ ...masterData, members: newMembers });
          setLastActionTarget(nickname.trim());
          showStatus(`${nickname}님이 마스터 명단에 추가되었습니다`, "success");
        }
      }));
    }
